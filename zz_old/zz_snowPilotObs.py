...
class SnowPilot_obs(object):
    ...
    def set_locationInfo(self, doc, root):
        ...
        elevation_val = (
            elevation_list[0].childNodes[1].firstChild.nodeValue
            if elevation_list[0].childNodes[1].firstChild
            else None
        )
        elevation_unit = (
            elevation_list[0].getAttribute("uom")
            if elevation_list[0].hasAttribute("uom")
            else None
        )
        ...
        aspect = (
            aspect_list[0].childNodes[1].firstChild.nodeValue
            if aspect_list[0].childNodes[1].firstChild
            else None
        )
        ...
        slopeAngle_val = (
            slopeAngle_list[0].childNodes[1].firstChild.nodeValue
            if slopeAngle_list[0].childNodes[1].firstChild
            else None
        )
        slopeAngle_unit = (
            slopeAngle_list[0].getAttribute("uom")
            if slopeAngle_list[0].hasAttribute("uom")
            else None
        )
